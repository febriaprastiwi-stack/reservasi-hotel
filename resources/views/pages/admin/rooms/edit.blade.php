...
<div class="container">
    ...
    <form action="{{ route('rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="number" name="jumlah_kasur" value="{{ $room->jumlah_kasur }}" class="form-control" min="1" required>
        </div>

        <div class="mb-3">
            <label>Harga per Malam</label>
            <input type="number" name="harga_per_malam" value="{{ $room->harga_per_malam }}" class="form-control" min="0" step="1000" required>
        </div>

        <div class="mb-3">
            <label>Gambar Kasur</label><br>
            @if ($room->gambar_kasur)
                <img src="{{ asset('storage/' . $room->gambar_kasur) }}" width="100" class="mb-2">
            @endif
